...
<body style="background-color:rgb(13, 82, 135);">
    ...
    <div class="container ">
        ...
        $qurry="SELECT * FROM `posts` ORDER BY id DESC limit 6";
        $d=mysqli_query($con,$qurry);
        if($d){
			if(mysqli_num_rows($d)>0){
				while($all = mysqli_fetch_assoc($d)){
          $pop="SELECT * FROM `likes` WHERE post_id =$all[id]";
          $w=mysqli_query($con,$pop);
          if($w){
                    ...
                    <div class="card  mt-3 mx-4 mb-3"  style="width: 18rem;background-color:rgb(8, 27, 55);box-shadow: 0 8px 32px 0 rgb(8, 27, 55);">
                        <div class="card-body">
                          ...
                          echo('
                            ...
                            echo("<button type='button' class='btn btn-outline-danger me-2' data-mdb-toggle='tooltip' data-mdb-placement='top' title='".$num."'>
                            <a href='increment_likes.php?post=$all[id]'>Like</a>
                           </button>");
